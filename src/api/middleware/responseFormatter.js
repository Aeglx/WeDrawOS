 ...
 */
export const sendSuccessResponse = (res, data = null, message = '操作成功', statusCode = 200) => {
 ...
 * @param {*} data - 可选的响应数据
 */
export const sendErrorResponse = (res, error = '操作失败', statusCode = 500, data = null) => {
  // 处理不同类型的错误输入
 ...
 */
export const sendValidationErrorResponse = (res, validationErrors) => {
  // 将验证错误数组转换为错误消息对象
  const errorDetails = {};
 ...
 * @param {*} message - 响应消息
 */
export const sendPaginatedResponse = (res, items, total, page = 1, limit = 20, message = '查询成功') => {
  // 计算总页数
 ...
 * JWT认证错误响应
 */
export const sendUnauthorizedResponse = (res, message = '认证失败，请重新登录') => {
  res.status(401).json({
    success: false,
    message,
    data: null,
 ...
 * 权限不足错误响应
 */
export const sendForbiddenResponse = (res, message = '权限不足，无法执行此操作') => {
  res.status(403).json({
    success: false,
    message,
    data: null,
 ...
 * 资源不存在错误响应
 */
export const sendNotFoundResponse = (res, message = '请求的资源不存在') => {
  res.status(404).json({
    success: false,
    message,
    data: null,
 ...
 * 内部服务器错误响应
 */
export const sendServerErrorResponse = (res, error = null) => {
 ...
 * 业务逻辑错误响应
 */
export const sendBusinessErrorResponse = (res, message, statusCode = 400, details = null) => {
  res.status(statusCode).json({
    success: false,
    message,
    data: null,
 ...
 * 批量操作响应格式化
 */
export const sendBatchOperationResponse = (res, successCount, failedCount, message = '批量操作完成') => {
  res.status(200).json({
    success: true,
    message,
    data: {
 ...
 * 生成API信息响应
 */
export const sendApiInfoResponse = (res, version = '1.0.0', name = 'WeDrawOS Customer Service API') => {
  res.status(200).json({
    success: true,
    message: 'API服务正常运行',
    data: {
 ...
 * 生成健康检查响应
 */
export const sendHealthCheckResponse = (res, databaseStatus = 'connected', services = {}) => {
 ...
 * 响应头设置中间件
 */
export const setupResponseHeaders = (req, res, next) => {
  // 设置内容类型
